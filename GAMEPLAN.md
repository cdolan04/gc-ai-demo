# Growth Capital CTO Take-Home: AI Shopify Agent

> **Purpose of this document**: Complete build plan for a Shopify embedded app that lets a store owner interact with their data through an AI agent. Serves as both context for a Claude Project (ask questions, iterate) and a roadmap for Claude Code (execute phase by phase). No code lives here — decisions, architecture, specs, and sequencing only.
